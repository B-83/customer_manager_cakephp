...
<!-- テーブル -->
    ...
    <?=$this->Form->hidden('customer[id]', ['value' => $customer['id']]) ?>
    ...
    <div class="table">
      <table class="table table-bordered table-sm table-hover table-list">
          ...
          <tr>
          	<th>名称</th>
            <td><?=$this->Form->text('customer[name]', ['default' => $customer['name'], 'id' => 'name', 'class' => 'width-650']) ?>
            	<span style="color: red; margin-left: 10px;"><?=$this->Form->error('name') ?></span>
            </td>
          </tr>
          <tr>
         	<th>かな</th>
            <td><?=$this->Form->text('customer[kana]', ['default' => $customer['kana'], 'id' => 'kana', 'class' => 'width-650']) ?>
            	<span style="color: red; margin-left: 10px;"><?=$this->Form->error('kana') ?></span>
            </td>
          </tr>
          <tr>
          	<th>郵便番号<small>&emsp;※半角数字</small></th>
            <td><?=$this->Form->text('customer[postal_code]', ['default' => $customer['postal_code'], 'id' => 'postal_code', 'class' => 'width-100']) ?>
          ...
          <tr>
          	<th>住所1</th>
            <td><?=$this->Form->select('customer[address1]', $states, ['empty' => '都道府県選択', 'default' => $customer['address1'], 'id' => 'address1', 'class' => 'table-form-select']) ?></td>
          </tr>
          <tr>
            <th>住所2</th>
            <td><?=$this->Form->text('customer[address2]', ['default' => $customer['address2'], 'id' => 'address2', 'class' => 'input-width-max']) ?></td>
          </tr>
          <tr>
            <th>電話番号<small>&emsp;※半角数字</small></th>
            <td><?=$this->Form->text('customer[tel]', ['default' => $customer['tel'], 'id' => 'tel', 'class' => 'width-150']) ?>
				<span style="color: red; margin-left: 10px;"><?=$this->Form->error('tel') ?></span>
			</td>
          </tr>
          <tr>
            <th>FAX番号<small>&emsp;※半角数字</small></th>
            <td><?=$this->Form->text('customer[fax]', ['default' => $customer['fax'], 'id' => 'fax', 'class' => 'width-150']) ?>
				<span style="color: red; margin-left: 10px;"><?=$this->Form->error('fax') ?></span>
			</td>
          </tr>
          <tr>
            <th>税区分</th>
            <td><?=$this->Form->select('customer[tax_type]', $taxTypes, ['default' => $customer['tax_type'], 'id' => 'tax_type', 'class' => 'table-form-select']) ?>
            	<span style="color: red; margin-left: 10px;"><?=$this->Form->error('tax_type') ?></span>
            </td>
          </tr>
          <tr>
            <th>丸め方法</th>
            <td><?=$this->Form->select('customer[rounding_type]', $roundingTypes, ['default' => $customer['rounding_type'], 'id' => 'rounding_type', 'class' => 'table-form-select']) ?>
            	<span style="color: red; margin-left: 10px;"><?=$this->Form->error('rounding_type') ?></span>
            </td>
          </tr>
          <tr>
            <th>締め日</th>
            <td><?=$this->Form->select('customer[closing_day]', $closingDays, ['default' => $customer['closing_day'], 'id' => 'closing_day', 'class' => 'table-form-select']) ?>
